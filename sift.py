...
def nonlinear_diffusion_filter(img, filter_size, n_iter, time_step=0.01):
    ...
    for i in range(n_iter):
        ...
        img = img + time_step * (
                    cv2.filter2D(grad_x * diffusivity, ddepth=-1, kernel=sobel_x) + cv2.filter2D(grad_y * diffusivity,
                                                                                                 ddepth=-1,
                                                                                                 kernel=sobel_y))
...
def load_image():
    ...
    display_image(image,
                  f"Original Image (Gray Scaled)\nWith Gaussian Noise Added\nMean: {mean}, Var: {variance}, Prob: {probability}",
                  canvas1)
